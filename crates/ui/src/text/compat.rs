...
impl TextView {
    ...
    /// Creates a text view that treats `text` as a single selectable paragraph.
    pub fn plain(id: impl Into<ElementId>, text: impl Into<SharedString>) -> Self {
        let id = id.into();
        Self {
            id: id.clone(),
            inner: gpui_base::TextView::plain(id, text).selectable(false),
            text_style: None,
        }
    }
...
/// Creates a plain-text view identified by the caller's code location.
#[track_caller]
